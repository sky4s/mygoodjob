...
namespace cms {
	...
	void DGLutFile::setTargetXYZVector(XYZ_vector_ptr targetXYZVector,
	    ...
	    initSheet(Target, headerCount, "Gray Level", "X", "Y (nit)",
		      ...
		      "Gamma R", "Gamma G", "Gamma B", "Gamma X",
		      "Gamma Y", "Gamma Z");
	    ...
	    double rMaxIntensity =
		componentLUT->getIntensity(Channel::R, 255);
	    double gMaxIntensity =
		componentLUT->getIntensity(Channel::G, 255);
	    double bMaxIntensity =
		componentLUT->getIntensity(Channel::B, 255);
		...
		if (null != dglut) {
		    ...
		    double rIntensity =
			componentLUT->getIntensity(Channel::R,
						   r) *
			targetWhiteRatio[0];
		    double gIntensity =
			componentLUT->getIntensity(Channel::G,
						   g) *
			targetWhiteRatio[1];
		    double bIntensity =
			componentLUT->getIntensity(Channel::B,
						   b) *
			targetWhiteRatio[2];
		    double X =
	...
	Component_vector_ptr DGLutFile::
	    getComponentVector(bool rgbFromGammaTable) {
	...
	void DGLutProperty::store(DGLutFile & dgfile) const {
		...
		if (true == c->feedbackFix) {
			...
			("defined dim - feedback fix init defect",
			 c->initDefectCount);
		    dgfile.addProperty("defined dim - feedback fix count",
				       c->feedbackFixCount);
		...
		if (true == c->dimFix || true == c->feedbackFix) {
		    dgfile.addProperty("defined dim - fix threshold",
				       c->dimFixThreshold);
		}

		XYZ_ptr blackXYZ =
		    (*c->componentVector)[c->componentVector->size() -
					  1]->XYZ;
		...
		dgfile.addProperty("absolute gamma start",
		...
		if (true == c->autoKeepMaxLumiParameter) {
		    dgfile.addProperty("auto keep max lumi adv parameter",
				       On);
		}
		dgfile.addProperty("keep max lumi adv over",
				   c->keepMaxLumiOver);
		dgfile.addProperty("keep max lumi adv gamma",
				   c->keepMaxLumiGamma);
		dgfile.addProperty("skip inverse b",
				   c->skipInverseB ? On : Off);
	...
	void DGLutProperty::storeAnalyzer(DGLutFile & dgfile,
		...
		dgfile.addProperty(TargetWhiteRatio,
				   *DoubleArray::toString(ratio));
	    }
	...
	xyY_ptr DGLutProperty::getReferenceColor(const string & prestring,
						 const Channel & ch) {
	    string_ptr value;
	    switch (ch.chindex) {
	    case ChannelIndex::R:
	...
	bptr < BitDepthProcessor > DGLutProperty::getBitDepthProcessor() {
		...
		bitDepth(new
			 BitDepthProcessor(in.bit, lut.bit, out.bit,
					   false));
	    return bitDepth;
	}
