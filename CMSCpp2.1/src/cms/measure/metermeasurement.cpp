...
namespace cms
{
  ...
  MeterMeasurement::MeterMeasurement(shared_ptr < Meter > meter, bool calibration):meter(meter), waitTimes(meter->getSuggestedWaitTimes()),
    ...
    {
      return measure0(nil_RGB_ptr, patchName, nil_string_ptr, nil_string_ptr,
                      false, true);
    };

    Patch_ptr MeterMeasurement::measure(RGB_ptr rgb,
  ...
  MeasureTool::MeasureTool(bptr < cms::measure::MeterMeasurement > mm):mm(mm),
                                              ...
                                              measureCondition)
    {
      if (!isAgingMode())
      {
        ...
        bptr < MeasureCondition >
          newMeasureCondition(new MeasureCondition(trueMeasureCode));
    ...
    Patch_vector_ptr MeasureTool::rampMeasure(bptr <
                                              cms::lcd::calibrate::
                                              MeasureCondition >
                                              measureCondition)
    {
      ...
      else
      {
        ...
        int originalWaitTimes = mm->WaitTimes;
        mm->WaitTimes = 5000;
        ...
        foreach(RGB_ptr rgb, *rgbMeasureCode)
        {
          Patch_ptr patch = mm->measure(rgb, rgb->toString());
          vector->push_back(patch);
          mm->WaitTimes = originalWaitTimes;
    ...
    Component_vector_ptr MeasureTool::flickerMeasure(bptr < MeasureCondition >
                                                     measureCondition)
    {
      RGB_vector_ptr rgbMeasureCode = measureCondition->getRGBMeasureCode();
      ...
      foreach(RGB_ptr rgb, *rgbMeasureCode)
      {
        ...
        Component_ptr component(new
                                Component(rgb, nil_RGB_ptr, patch->getXYZ()));
    ...
    int MeasureTool::getMaxZDGCode(bptr <
                                   ...
                                   bptr < BitDepthProcessor > bitDepth)
    {
      ...
      bptr < MeasureCondition >
        measureCondition(new MeasureCondition(bitDepth));
      int maxZDGCode = mt->getMaxZDGCode(measureCondition);
      return maxZDGCode;
    };
    int MeasureTool::getMaxBIntensityRawGrayLevel(bptr < MeterMeasurement >
                                                  mm,
