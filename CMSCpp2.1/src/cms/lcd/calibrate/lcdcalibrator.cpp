...
namespace cms {
	...
	namespace calibrate {
		...
		case SecondWhite::DeHook2:
			...
			(lcdmodel4DeHook, this->maxBRawGrayLevel, 0.003);
		...
		if (keepMaxLuminance ==
		    ...
		    double maxLuminance =
			(*originalComponentVector)[0]->XYZ->Y;
			...
			firstAnalyzer->getReferenceColor()->Y;
		...
		double maxLuminance =
		    (*luminanceGammaCurve)[luminanceGammaCurve->size() -
					   1];
		...
		case KeepMaxLuminance::NativeWhite:{
			...
			bptr < DGLutOp >
			    deHook2(new
				    DeHook2Op(bitDepth, maxBDGGrayLevel));
			dgop.addOp(deHook2);
		    }
		    break;
		...
		if (dimFix) {
		    //50量到0
		    Component_vector_ptr dimComponentVector =
