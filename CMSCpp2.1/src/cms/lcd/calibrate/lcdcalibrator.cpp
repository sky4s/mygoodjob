...
namespace cms {
	...
	namespace calibrate {
		...
		for (int x = 0; x <= dimGammaEnd; x++) {
		    ...
		    double gamma =
			Interpolation::linear(0, dimGammaEnd, dimGamma,
					      brightGamma, x);
		    double v = Math::pow(normal, gamma);
		    (*result)[x] = v;
		}
		...
		int effectiven = bitDepth->getEffectiveInputLevel();
		setGammaCurve0(getGammaCurveVector
			       (dimGamma, dimGammaEnd, middleGammaEnd,
				gamma, n, effectiven));
		useGammaCurve = false;
		rgbIndepGamma = false;
	    };
	     ...
	     */
	    void LCDCalibrator::
		setKeepMaxLuminanceSmooth2NativeWhite(int over,
						      bool autoParameter) {
		...
		LCDCalibrator::getFirstAnalzyer() {
		bptr < IntensityAnalyzerIF > firstAnalyzer =
	     ...
	     */
	    void LCDCalibrator::
		init2ndWhiteAnalyzer(KeepMaxLuminance keepMaxLuminance,
				     DeHook deHook) {
		SecondWhite secondWhite =
		    DeHookProcessor::getSecondWhite(keepMaxLuminance,
						    deHook);
		//=====================================================
		...
		case SecondWhite::DeHook2:
		     ...
		     */
		    //*must measure
		    this->maxBRawGrayLevel =
			processor.getMaxBIntensityRawGrayLevel();
		    if (-1 == maxBRawGrayLevel) {
			ShowMessage("-1 == maxBRawGrayLevel");
		    }
		    lcdmodel4DeHook =
			processor.getLCDModelForDeHook(this->
						       maxBRawGrayLevel);
			...
			this->maxBDGGrayLevel =
			    processor.
			    getReasonableGammaGrayLevel(patchVector,
							gammaDWLimit,
							gammaUPLimit);
			if (-1 == this->maxBDGGrayLevel) {
			    this->maxBDGGrayLevel = max - 1;
			}
			if (this->maxBDGGrayLevel <=
			    bGap1stGammaBoundGrayLevel) {
			    this->maxBDGGrayLevel =
				bGap1stGammaBoundGrayLevel + 1;
			}

			double normalOutput =
			    (*gammaCurve)[this->maxBDGGrayLevel];
			int maxcount = bitDepth->getInputMaxDigitalCount();
			double normalInput =
			    ((double) this->maxBDGGrayLevel) / maxcount;
			double gamma =
			    GammaFinder::getGammaExponential(normalInput,
							     normalOutput);
			//再從這裡面撈出最接近target gamma的
			bestGammaPatch1 =
			    processor.getBestGammaPatch(patchVector, gamma,
							this->
							maxBDGGrayLevel);
		...
		secondWhiteAnalyzer =
		    MaxMatrixIntensityAnalyzer::getReadyAnalyzer(mm,
								 redmax,
								 greenmax,
								 bluemax);
	    };
		...
		if (true == useNewMethod) {
		    ...
		    //舊方法融合色度以及DG Code的處理
		    DGLutGenerator generator(originalComponentVector,
					     keepMaxLuminance);
		    dglut =
			oldMethod(generator, panelRegulator,
				  quantizationBit);
		}
		...
		LCDCalibrator::initAdvancedDGLutGenerator() {
	    ...
	    double_vector_ptr LCDCalibrator::
		initLuminanceGammaCurve(XYZ_ptr referenceXYZ) {
		//==========================================================
		// 產生gamma curve用
		//==========================================================
		//max luminance
	     ...
	     */
	    RGB_vector_ptr LCDCalibrator::newMethod(bptr < PanelRegulator >
						    panelRegulator) {
		...
		//=============================================================
		double_vector_ptr luminanceGammaCurve =
		    initLuminanceGammaCurve(referenceXYZ);
		if (nil_double_vector_ptr == luminanceGammaCurve) {
		    return nil_RGB_vector_ptr;
		}
	...
	advgenerator->setPanelRegulator(panelRegulator);
		...
		for (; overParameter >= minOverParameter;
		    ...
		    this->multiGenTargetXYZVector =
			advgenerator->MultiGenTargetXYZVector;
		    ...
		    //=========================================================
		    if (true == feedbackFix
			&& (true == debugMode || bitDepth->isTCONInput()
			    || pcWithTCONInput)) {
			//先從DGLut算出灰階的dx dy
			//再從這樣的結果微調目標值
	    ...
	    void LCDCalibrator::
		fixChromaticityReverseByFeedback(RGB_vector_ptr dglut) {
		feedbackFixer =
		    bptr < FeedbackFixer >
		    (new
	    ...
	    void LCDCalibrator::storeInfo2DGLutFile(bptr <
						    cms::colorformat::
						    DGLutFile >
						    dglutFile) {
		DGLutProperty property(this);
		...
		if (null != originalComponentVector) {
		    //寫入raw data
		    dglutFile->setRawData(originalComponentVector,
					  initialRGBGamma, finalRGBGamma);
		}
		...
		if (null != multiGenTargetXYZVector) {
		    dglutFile->
			setMultiGenTargetXYZVector(multiGenTargetXYZVector,
						   dglut, bitDepth);
		}
		...
		if (null != mm && FakeMode::None == mm->FakeMeasureMode) {
			...
			mm->MeasurePatchVector;
		    if (null != measurePatchVector
			&& measurePatchVector->size() != 0) {
		...
		case KeepMaxLuminance::NativeWhite:{
		    ...
		    if (SecondWithBGap1st == dehook
			|| SecondWithBGap1st == dehook) {
			RGB_ptr rgb =
			    secondWhiteAnalyzer->getReferenceRGB();
			if (-1 == maxBDGGrayLevel) {
			    ShowMessage("-1 == maxBDGGrayLevel");
			}
			bptr < DGLutOp >
			    deHook2(new
				    DeHook2Op(bitDepth, rgb,
					      maxBDGGrayLevel));
			dgop.addOp(deHook2);
		    }
		    break;
		...
		if (true == absGamma) {
			...
			static_cast <
			double >(absGammaStartGL) / (effectiven - 1);
		    double startAbsoluteNormalOutput =
			GammaFinder::gamma(startNormalInput,
					   startGLAboveGamma);
		    double startAbsoluteLuminance =
			maxLuminance * startAbsoluteNormalOutput;
		    //=========================================================
		    // 2
		    //=========================================================
			...
			(startAbsoluteLuminance -
			 minLuminance) / (maxLuminance - minLuminance);
		    double relativeGamma =
			GammaFinder::getGammaExponential(startNormalInput,
			...
			double absoluteNormalOutput =
			    (*normalGammaCurve)[x];
			double absoluteGammaLuminance =
			    maxLuminance * absoluteNormalOutput;

			double relativeNormalOutput =
			    GammaFinder::gamma(normalInput, relativeGamma);
			double relativeGammaLuminance =
			    (maxLuminance -
			     minLuminance) * relativeNormalOutput +
			    minLuminance;
			if (absoluteGammaLuminance >
			    relativeGammaLuminance) {
			    turnGrayLevel = x;
			    break;
			}
		    }
		    ...
		    //=========================================================
		    double_vector_ptr newNormalGammaCurve(new
							  double_vector());
			...
			double relativeNomralOutput =
			    GammaFinder::gamma(normalInput, relativeGamma);
			newNormalGammaCurve->
			    push_back(relativeNomralOutput);
		    }
		    //絕對gamma區間
			...
			double absoluteNormalOutput =
			    (*normalGammaCurve)[x];
			double absoluteGammaLuminance =
			    absoluteNormalOutput * maxLuminance;
			double relativeNormalOutput =
			    (absoluteGammaLuminance -
			     minLuminance) / (maxLuminance - minLuminance);
			...
			newNormalGammaCurve->
			    push_back(relativeNormalOutput);
			...
			double absoluteNormalOutput =
			    (*normalGammaCurve)[x];
			newNormalGammaCurve->
			    push_back(absoluteNormalOutput);
		    }
		    return getLuminanceGammaCurve(newNormalGammaCurve,
						  maxLuminance,
						  minLuminance);
		} else {
