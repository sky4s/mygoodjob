...
void __fastcall TGammaMeasurementForm::FormShow(TObject * Sender)
{
    ...
    if(bitDepth->isAgingMode()) {
        ...
        this->Label5->Visible = false;            // Aging mode⊿ノ留旅癬ㄓ
        this->Label6->Visible = false;            // 秖代计CheckBox_10BitInMeasurement∕﹚
...
{
    bptr < cms::measure::IntensityAnalyzerIF > analyzer = MainForm->initAnalyzer();
    if (null != analyzer) {
...
void __fastcall TGammaMeasurementForm::Button_LoadDGTableClick(TObject * Sender)
{
	...
	//checkbox翴匡穦∕﹚秖代end level, ┮璶耚硂よ
