...
namespace math {
	...
	if (normalizeInput->size() != normalizeOutput->size()) {
		...
		("normalizeInput->size()!= normalizeOutput->size()");
	}
	...
	for (unsigned int x = 0; x < normalizeInput->size(); x++) {
	    ...
	    numerator +=
		Math::log((*normalizeInput)[x]) *
		Math::log((*normalizeOutput)[x]);
	}
    ...
    double_vector_ptr GammaFinder::normalize(double_vector_ptr valueArray,
					     double minValue,
					     double maxValue) {
	double_vector_ptr normal =
	    DoubleArray::minus(valueArray, minValue);
	...
	double_vector_ptr normalInput =
	    normalize(input, inputMinValue, inputMaxValue);
	double_vector_ptr normalOutput =
	    normalize(output, outputMinValue, outputMaxValue);
