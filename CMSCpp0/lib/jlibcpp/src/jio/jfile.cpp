...
namespace       jio {
    ...
    File::File(std::string filename_, int flags_):jcommon::Object() {
	...
	if ((flags_ & F_CREATE) == 0) {
	    _fd =
		CreateFile(filename_.c_str(), opt,
	...
	if (_fd == INVALID_HANDLE_VALUE) {
	    ...
	    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, 0,	// no 
												// 
			  // source 
			  // buffer 
			  // needed
	...
	if (_type == F_DIRECTORY) {
	    ...
	    if (_dir == NULL) {
		// throw std::runtime_error("Directory cannot be
		// accessed");
  ...
  File::File(std::string prefix, std::string sufix, bool scramble):
...
#ifdef _WIN32
	int             r =
	    1 + (int) (1000000.0 * (rand() / (RAND_MAX + 1.0)));
	...
	_fd =
	    CreateFile(_filename.c_str(), GENERIC_READ | GENERIC_WRITE,
		       FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
		       CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
	...
	_fd =
	    open(_filename.c_str(),
		 O_RDWR | O_LARGEFILE | O_CREAT | O_EXCL,
		 S_IREAD | S_IWRITE | S_IRGRP | S_IROTH);

	if (_fd == -1) {
	    _fd =
		open(_filename.c_str(), O_RDWR | O_LARGEFILE,
		     S_IREAD | S_IWRITE | S_IRGRP | S_IROTH);
    ...
    File           *File::CreateTemporary(std::string prefix,
					  std::string sufix) {
	File           *file = new File(prefix, sufix, true);	// F_READ_WRITE 
								// 
	// |
	// F_LARGEFILE 
	// |
	...
	return true;
#else
	if (S_ISREG(_stat.st_mode) | S_ISFIFO(_stat.st_mode) |
	...
	DWORD           n =::GetCurrentDirectory((DWORD) sizeof(buffer),
						 (LPSTR) buffer);
	...
	if (extension == "") {
		...
		if (file.size() > extension.size()) {
		    if (strcmp
			((const char *) (file.c_str() - extension.size()),
			 extension.c_str()) == 0) {
			l->push_back(file);
		    }
		}
    ...
    void            File::Remove() {
	// CHANGE:: posso trocar por remove(3) que jah verifica se eh
	// dirtorio ou arquivo
