...
namespace       jcommon {
	...
	if (NULL == mCurrent) {
		...
		if (JSONNode::eObject == parent->getType()) {
		    JSONObjectNode *object =
			static_cast <
			JSONObjectNode * >((JSONNode *) parent);
  ...
  :	mType(type) {
	...
	if (JSONNode::eObject == mParent->getType()) {
	    ...
	    JSONObjectNode *object =
		(JSONObjectNode *) mParent->getParent();
  ...
  :	JSONNode(eObject) {
	...
	const void     *lastChild =
	    mValueList->getItem(JSONArrayList::LAST_INDEX);
  ...
  :	JSONNode(eArray) {
	...
	for (int pos = 0; pos < len && NULL == mParser->getError();
	     pos += 64) {
	...
	for (JSONPullEvent * event = mParser->getNext();
		...
		{
		    JSONStringNode *newString =
			new JSONStringNode(((JSONTextEvent *) event)->
					   getText());
		    addNode(newString);
		    break;
		}
	    ...
	    case JSONPullEvent::eBoolean:
		{
			...
			((JSONTextEvent *) event)->getText();
#ifndef _BCB
		    JSONBooleanNode *newBoolean =
			new JSONBooleanNode(0 == strcasecmp(text, "true"));
#else
		    JSONBooleanNode *newBoolean =
			new JSONBooleanNode(0 == strcmpi(text, "true"));
#endif
		    addNode(newBoolean);
		    break;
		}
	    case JSONPullEvent::eComment:
		{
		    ...
		    JSONCommentNode *newComment =
			new JSONCommentNode(text);
		    addNode(newComment);
		    break;
		}
	...
	for (int i = 0; i < objectNode->getCount(); i++) {
		...
		&& JSONNode::eComment != iter->getType())
		buffer->append(',');
	}
	...
	if (NULL != event) {
	    if (JSONPullEvent::eStartObject == event->getEventType())
		mLevel++;
	    if (JSONPullEvent::eEndObject == event->getEventType())
		mLevel--;
	}
	...
	if (NULL != event) {
		...
		if (mObjectCount <= mNameList->getCount()) {
		    snprintf(errmsg, sizeof(errmsg),
			     "miss value for name \"%s\"",
			     (char *) mNameList->
			     getItem(JSONArrayList::LAST_INDEX));
		} else {
		    char           *last =
			(char *) mStack->
			takeItem(JSONArrayList::LAST_INDEX);
		    if (NULL != last) {
			if ('o' != *last) {
			    snprintf(errmsg, sizeof(errmsg),
				     "mismatched object, start-with<%s>",
				     last);
			}
		    } else {
			snprintf(errmsg, sizeof(errmsg),
				 "mismatched object, start-with<NULL>");
		    }
		...
		char           *last =
		    (char *) mStack->takeItem(JSONArrayList::LAST_INDEX);
		if (NULL != last) {
		    if ('a' != *last) {
			snprintf(errmsg, sizeof(errmsg),
				 "mismatched array, start-with<%s>", last);
		    }
		} else {
		    snprintf(errmsg, sizeof(errmsg),
			     "mismatched array, start-with<NULL>");
		}

		if (isObjectParent())
		    free(mNameList->takeItem(JSONArrayList::LAST_INDEX));
	    } else if (JSONPullEvent::eName == eventType) {
		mNameList->
		    append(strdup(((JSONNameEvent *) event)->getText()));
	...
	if (NULL != error) {
		...
		if (mErrorIndex < (int) sizeof(mErrorSegment)) {
		    ...
		    int             offset =
			mErrorIndex % sizeof(mErrorSegment);
		    strncpy(temp, mErrorSegment + offset,
			    sizeof(mErrorSegment) - offset);
		    strncpy(temp + sizeof(mErrorSegment) - offset,
			    mErrorSegment, offset);
		}

		for (char *pos = temp, *dest = segment; '\0' != *pos;
		     pos++) {
		    if ('\r' == *pos) {
			*dest++ = '\\';
			*dest++ = 'r';
	    ...
	    snprintf(msg, sizeof(msg),
		     "%s ( occured at row(%d), col(%d) : %s )", error,
		     mRowIndex + 1, mColIndex + 1, segment);
	...
	if (isObjectParent() && mObjectCount > mNameList->getCount())
	    ret = 1;
	...
	if (mStack->getCount() > 0) {
	    char           *last =
		(char *) mStack->getItem(JSONArrayList::LAST_INDEX);
	    if ('o' == *last)
		ret = 1;
	}
    ...
    int             JSONCodec::encode(double value,
				      JSONStringBuffer * outBuffer) {
	char            buffer[128] = { 0 };
    ...
    void            JSONReader::changeReader(JSONPullParser * parser,
					     JSONReader * reader) {
	// printf( "\nchange: %s\n", typeid( *reader ).name() );
	parser->changeReader(reader);
    }

    void            JSONReader::setError(JSONPullParser * parser,
					 const char *error) {
	...
	if (isspace(c)) {
	    ...
	    JSONReader     *reader =
		new JSONCommentReader(new JSONSpaceReader(mHasReadComma));
		...
		if (isObjectParent(parser)) {
		    ...
		    if ('{' == c) {
			JSONReader     *reader =
			    new JSONStartObjectReader();
			changeReader(parser, reader);
			reader->read(parser, c);
		    } else if ('[' == c) {
			JSONReader     *reader =
			    new JSONStartArrayReader();
			changeReader(parser, reader);
			reader->read(parser, c);
		    } else {
    ...
    void            JSONStartObjectReader::read(JSONPullParser * parser,
						char c) {
	if (isspace(c)) {
	    // skip
	    ...
	    JSONReader     *reader =
		new JSONCommentReader(new
				      JSONStartObjectReader
				      (mHasReadLeftBrace));
	    changeReader(parser, reader);
	    reader->read(parser, c);
	} else {
    ...
    JSONPullEvent  *JSONStartObjectReader::getEvent(JSONPullParser *
						    parser) {
    ...
    void            JSONEndObjectReader::read(JSONPullParser * parser,
					      char c) {
	if ('}' == c) {
    ...
    void            JSONStartArrayReader::read(JSONPullParser * parser,
					       char c) {
    ...
    void            JSONEndArrayReader::read(JSONPullParser * parser,
					     char c) {
	if (']' == c) {
	    changeReader(parser, new JSONSpaceReader());
	} else {
	...
	if (isspace(c)) {
	    ...
	    JSONReader     *reader =
		new JSONCommentReader(new JSONValueReader());
	    changeReader(parser, reader);
	    reader->read(parser, c);
	} else if ('{' == c) {
    ...
    void            JSONBooleanReader::read(JSONPullParser * parser,
					    char c) {
	if (!isalpha(c)) {
	    JSONReader     *reader = new JSONSpaceReader();
	    changeReader(parser, reader);
    ...
    void            JSONCommentReader::read(JSONPullParser * parser,
					    char c) {
    ...
    void            JSONPaddingReader::read(JSONPullParser * parser,
					    char c) {
	if (!isspace(c)) {
	...
	sort(int (*cmpFunc) (const void *, const void *)) {
	...
	if (mCount >= mMaxCount) {
	    mMaxCount = (mMaxCount * 3) / 2 + 1;
	    void          **newEntries =
		(void **) malloc(sizeof(void *) * mMaxCount);
	    ...
	    memcpy(newEntries, &(mEntries[mHead]),
		   sizeof(void *) * headLen);
	    if (tailLen) {
		memcpy(&(newEntries[headLen]), mEntries,
		       sizeof(void *) * tailLen);
	    }
