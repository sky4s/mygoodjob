...
#include <colorip/11307/DCR/DCR_11307.h>
...
//---------------------------------------------------------------------------
void __fastcall TDCRForm1::DCR_ScrollBar_Scroll(TObject * Sender, TScrollCode ScrollCode,
						int &ScrollPos)
{
...
//---------------------------------------------------------------------------
void __fastcall TDCRForm1::DCR_ScrollBar2_Scroll(TObject * Sender, TScrollCode ScrollCode,
						 int &ScrollPos)
{
    TCheckBox *c = (TCheckBox *) Sender;
    int idx = StrToInt(c->Hint);
...
void __fastcall TDCRForm1::DCRLblE3_KeyPress(TObject * Sender, char &Key)
{
	...
	val2 =
	    (set_val / DCRLblE3[idx]->Addr.Divide2()) % (int) pow(2,
								  DCRLblE3[idx]->Addr.Byte2.
								  BitNum());
...
//---------------------------------------------------------------------------
void __fastcall TDCRForm1::ALS_ScrollBar_Scroll(TObject * Sender, TScrollCode ScrollCode,
						int &ScrollPos)
{
    TCheckBox *c = (TCheckBox *) Sender;
    int idx = StrToInt(c->Hint);
...
//----------------------------------------------------------------------------
void __fastcall TDCRForm1::HDR_ScrollBar_Scroll(TObject * Sender, TScrollCode ScrollCode,
						int &ScrollPos)
{
    TCheckBox *c = (TCheckBox *) Sender;
    int idx = StrToInt(c->Hint);
...
//----------------------------------------------------------------------------
void __fastcall TDCRForm1::HDR_ScrollBar2_Scroll(TObject * Sender, TScrollCode ScrollCode,
						 int &ScrollPos)
{
    TCheckBox *c = (TCheckBox *) Sender;
    int idx = StrToInt(c->Hint);
...
void __fastcall TDCRForm1::Btn_DCR_reloadClick(TObject * Sender)
{
	...
	if (DCRScrlB2[i]->ScrlB->Visible == true) {
	    ...
	    DCRScrlB2[i]->ScrlB->Position =
		(int) read_val1 *DCRScrlB2[i]->Addr.Divide() + read_val2;
	...
	if (HDRScrlB2[i]->ScrlB->Visible == true) {
	    ...
	    HDRScrlB2[i]->ScrlB->Position =
		(int) read_val1 *HDRScrlB2[i]->Addr.Divide() + read_val2;
...
void __fastcall TDCRForm1::FormKeyDown(TObject * Sender, WORD & Key, TShiftState Shift)
{
    if (Key == 0x40)
