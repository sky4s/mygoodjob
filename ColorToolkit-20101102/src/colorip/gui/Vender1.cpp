...
//---------------------------------------------------------------------------
void __fastcall TVenderForm1::FormClose(TObject * Sender, TCloseAction & Action)
{
    delete[]OVender;
...
void __fastcall TVenderForm1::Btn_vender_reloadClick(TObject * Sender)
{
	...
	if (VendScrlB2[i]->ScrlB->Visible == true) {
	    ...
	    VendScrlB2[i]->ScrlB->Position =
		(int) read_val1 *VendScrlB2[i]->Addr.Divide() + read_val2;
	    VendScrlB2[i]->StTxt->Caption =
		(int) read_val1 *VendScrlB2[i]->Addr.Divide() + read_val2;
	}
    }
    Vend_Chg = 1;
...
void __fastcall TVenderForm1::FormKeyDown(TObject * Sender, WORD & Key, TShiftState Shift)
{
    if (Key == 0x40) {
