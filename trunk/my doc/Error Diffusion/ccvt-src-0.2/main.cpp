...
//  printf("%f\n",n);
  for (int x = 0; x < n; ++x) {
    for (int y = 0; y < n; ++y) {
      float dx = x / n * torusSize;
      float dy = y / n * torusSize;
...
int main(int, char*[]) {
	...
	for(int err=1;err<=8;err++){
	  ...
	  //const char*   RESULT_FILENAME   = "error1.eps";
	  //char*   RESULT_FILENAME   = ('0'+err)+".eps";
	  //std::string eps("eps");
	  //char* RESULT_FILENAME =  ('0'+err);
	  sprintf (RESULT_FILENAME, "err%d.eps", err);
	  const double  RESULT_RADIUS     = 1;
	  
	  typedef Optimizer<Site<Point2>, Point2, MetricToroidalEuclidean2> Optimizer;
	
	  // intializing the underlying discrete space
	  printf("initialization...points\n");
	  Point2::List points;
	  if (CONSTANT_DENSITY) {
	  ...
	  printf("initialization...sites\n");
	  unsigned int overallCapacity = static_cast<int>(points.size());
	  Site<Point2>::List sites;
	  for (int i = 0; i < NUMBER_SITES; ++i) {
	    float x = static_cast<float>(rand() % RAND_MAX) / RAND_MAX * TORUS_SIZE;
	    float y = static_cast<float>(rand() % RAND_MAX) / RAND_MAX * TORUS_SIZE;
	    int capacity = overallCapacity / (NUMBER_SITES - i);
	    overallCapacity -= capacity;
	    sites.push_back(Site<Point2>(i, capacity, Point2(x, y)));
	  }
	  
//	  double n = sqrt(static_cast<double>(NUMBER_POINTS));
//	  int i=0;
//  	  for (int x = 0; x < n; ++x) {
//      	for (int y = 0; y < n; ++y) {
//	      double dx = x / n * TORUS_SIZE;
//	      double dy = y / n * TORUS_SIZE;
////	      printf("%d %f %f\n",i,dx,dy);
////	      points.push_back(Point2(dx, dy));
//	      
//	      int capacity = overallCapacity / (NUMBER_SITES - i);
////	      printf("%d\n",capacity);
//	      overallCapacity -= capacity;
//	      sites.push_back(Site<Point2>(i, capacity, Point2(dx, dy)));
//	      
//	      printf("%d %d\n",i,NUMBER_SITES);
//	      i++;
// 
