...
#pragma resource "*.dfm"
TSharpnessV2_1Form *SharpnessV2_1Form;
static String SP = "Sharpness";
static String CE = "Contrast";
//---------------------------------------------------------------------------
__fastcall TSharpnessV2_1Form::TSharpnessV2_1Form(TComponent* Owner):TForm(Owner)
{
    SP_Chg = 0;
    gain_flag = false;
    SP_IsChkSum = false;
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::FormClose(TObject * Sender, TCloseAction & Action)
{
    delete[]OSP;
    delete[]cb;
    delete[]ChkB;
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::ScrollBar_Change(TObject * Sender)
{
    if (SP_Chg == 0) {
	return;
    }
    ...
    if (ScrlB[idx]->Addr.Name().AnsiCompare("GLB_STR") == 0) {
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::CheckBox_Click(TObject * Sender)
{
    if (SP_Chg == 0) {
	return;
    }
...
void __fastcall TSharpnessV2_1Form::ComboBox_Click(TObject * Sender)
{
    if (SP_Chg == 0) {
	return;
    }
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::LblE3_KeyPress(TObject * Sender, char &Key)
{
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::FormCreate(TObject * Sender)
{
    ...
    ChkB[6]->Chkb = CheckBox7;  //I3D_EDGE_EN->IMGEN_SP
    ...
    CboB[6]->Cbob = ComboBox7;
    CboB[6]->CbobL = Label27;
    ...
    ScrlB[16]->Lbl = Label25;
    ScrlB[16]->StTxt = StaticText17;
    ScrlB[16]->ScrlB = ScrollBar17;
    ScrlB[17]->Lbl = Label26;
    ScrlB[17]->StTxt = StaticText18;
    ScrlB[17]->ScrlB = ScrollBar18;
...
//---------------------------------------------------------------------------
void TSharpnessV2_1Form::SP_LUT_FuncEnable(bool flag_en)
{
    if (flag_en == true) {
...
//----------------------------------------------------------------------------
void TSharpnessV2_1Form::Clear_LUT(bool type)
{
    if (type == true) {
	for (int i = 0; i < 32; i++) {
	    SP_lut[i] = -1;
...
//--------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::SP_LUTMouseDown(TObject * Sender,
						  TMouseButton Button,
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::SP_LUTDblClick(TObject * Sender)
{
    int t;
    for (int i = 0; i < 32; i++) {
	if (SP_lut[i] == -1) {
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::sb_softgainChange(TObject * Sender)
{
    if (SP_lut[0] != -1) {
	gain_flag = true;
	float gain = 15 - (float) sb_softgain->Position / 10;  //原本最上面才為零,所以改用扣的讓最下面為零
	...
	sb_softgain->Position = 140;    //因為最上面為零, 所以改位置10->140
    }

}

//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_sp_lut_SetClick(TObject * Sender)
{
    SP_LUT_FuncEnable(0);
    SP_LUT->Canvas->Refresh();
    Clear_LUT(true);
    LUT_type->ItemIndex = -1;
    sb_softgain->Position = 140;
    st_softgain->Caption = " 1 ";
    SP_LUT_FuncEnable(1);
}

//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_GainSetClick(TObject * Sender)
{
    if (SP_lut[0] != -1) {
	gain_flag = false;
	for (int i = 0; i < 32; i++) {
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_sp_lut_writeClick(TObject * Sender)
{
    SP_Chg = 0;
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_sp_lut_readClick(TObject * Sender)
{
    SP_Chg = 0;
    if (SP_addr[0].LutNum() != 32) {
	ShowMessage(" SP LUT number defined in AddressSP.cpp is wrong.");
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::Btn_SP_reloadClick(TObject * Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_sp_LoadClick(TObject * Sender)
{
    Clear_LUT(true);
...
//---------------------------------------------------------------------------
bool TSharpnessV2_1Form::Load_SP(String Fpath)
{
    ...
    ini.readComboBox(SP, "MAG_TH_6", ComboBox7);
    ...
    ini.readScrollBar(CE, "PIX_FILTER_DARK", ScrollBar8);
    ini.readScrollBar(CE, "PIX_FILTER_BRIGHT", ScrollBar9);
    ini.readScrollBar(CE, "DARK_OFS", ScrollBar13);
    ini.readScrollBar(CE, "BRIGHT_OFS", ScrollBar14);
    ini.readScrollBar(CE, "CE_STR", ScrollBar12);
    ini.readScrollBar(CE, "DARK_MAX_ADJ", ScrollBar10);
    ini.readScrollBar(CE, "DARK_DR", ScrollBar15);
    ini.readScrollBar(CE, "BRIGHT_MAX_ADJ", ScrollBar11);
    ini.readScrollBar(CE, "BRIGHT_DR", ScrollBar16);
    ini.readScrollBar(CE, "STEP_GAIN", ScrollBar17);
    ini.readScrollBar(CE, "VARIANT_TH", ScrollBar18);
    // CheckBox
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::btn_sp_SaveClick(TObject * Sender)
{
    ...
    ini.writeComboBox(SP, "MAG_TH_6", ComboBox7);
    ...
    ini.writeScrollBar(CE, "PIX_FILTER_DARK", ScrollBar8);
    ini.writeScrollBar(CE, "PIX_FILTER_BRIGHT", ScrollBar9);
    ini.writeScrollBar(CE, "DARK_OFS", ScrollBar13);
    ini.writeScrollBar(CE, "BRIGHT_OFS", ScrollBar14);
    ini.writeScrollBar(CE, "CE_STR", ScrollBar12);
    ini.writeScrollBar(CE, "DARK_MAX_ADJ", ScrollBar10);
    ini.writeScrollBar(CE, "DARK_DR", ScrollBar15);
    ini.writeScrollBar(CE, "BRIGHT_MAX_ADJ", ScrollBar11);
    ini.writeScrollBar(CE, "BRIGHT_DR", ScrollBar16);
    ini.writeScrollBar(CE, "STEP_GAIN", ScrollBar17);
    ini.writeScrollBar(CE, "VARIANT_TH", ScrollBar18);
    // CheckBox
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::LUT_typeClick(TObject * Sender)
{
    String Fpath;
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::FormKeyDown(TObject * Sender, WORD & Key, TShiftState Shift)
{
    if (Key == 0x40) {
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::Edit_ResolutionHKeyPress(TObject * Sender, const char &Key)
{
...
//---------------------------------------------------------------------------
void __fastcall TSharpnessV2_1Form::FormKeyPress(TObject * Sender, char &Key)
{
