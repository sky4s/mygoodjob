  ...
  namespace colorspace
  {
    namespace depend
    {
      ...
      double_array DeviceDependentSpace::
        XYZ2LinearRGBValues(double_array XYZValues,
                                                  ...
                                                  colorSpace)
      {
        double_array rgbValues =
          XYZ2LinearRGBValues(XYZ->getValues(), colorSpace);
      ...
      Channel_vector_ptr Channel::
        getBesidePrimaryChannel(const Channel & primaryColorChannel)
      {
                          ...
                          RGBColorSpace rgbColorSpace)
      {
        ...
        if (rgbColorSpace == RGBColorSpace::sRGB)
        {
          for (int x = 0; x < 3; x++)
          {
            ...
            {
              rgbValues[x] =
                1.055 * Math::pow(rgbValues[x], 1. / 2.4) - 0.055;
            }
            if (negative)
            {
              rgbValues[x] = -rgbValues[x];
      ...
      double_array RGBBase::linearToRGBValues(double_array linearRGBValues,
                                              double gamma)
      {
      ...
      double_array RGBBase::toLinearRGBValues(double_array rgbValues,
                                              double gamma)
      {
      ...
      RGBBasis::RGBBasis(const RGBColorSpace & colorSpace,
                         double_array values):rgbColorSpace(&colorSpace)
      {
        if (null != values)
        {
    ...
    RGBBasis::RGBBasis(RGB_ptr rgb):rgbColorSpace(&rgb->
                    getRGBColorSpace())
      {
    ...
    ColorAppearanceBase::ColorAppearanceBase(RGB_ptr rgb, int_array lshIndex):RGBBasis(rgb),
      ...
      HSV::HSV(const RGBColorSpace & colorSpace,
               double_array hsvValues):ColorAppearanceBase(colorSpace,
                                                           hsvValues,
                                                           lshIndex)
      {
      };
      HSV::HSV(const RGBColorSpace & colorSpace, double h, double s,
               double v):ColorAppearanceBase(colorSpace, getValuesArray(h, s,
                                                                        v),
                                             lshIndex)
      {
    ...
    HSV::HSV(RGB_ptr rgb):ColorAppearanceBase(rgb, lshIndex)
      ...
      RGB_ptr HSV::toRGB()
      {
        ...
        RGB_ptr rgb(new
                    RGBColor(*rgbColorSpace, rgbValues, MaxValue::Double255));
        return rgb;
      };
