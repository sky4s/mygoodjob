	...
	namespace calibrate {
	  ...
	  LCDCalibrator::LCDCalibrator(bptr < ComponentFetcher > fetcher, bptr < BitDepthProcessor > bitDepth):bitDepth(bitDepth)
		...
		for (int x = 1; x <= 2; x++) {
		    ...
		    //外層全量
		    while ((reverseIndex = checkReverse(dxofBase, 1, checkEnd)) != -1) {
			...
			do {
			    ...
			    deltaOfBase = selectDelta(dxofBase, dyofBase, ch);
			} while (checkReverse(deltaOfBase, 1, 2) != -1);
