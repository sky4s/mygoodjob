...
namespace cms
{
  namespace util
  {
    ...
    bool Util::deleteExist(const std::string & filename)
    {
      return deleteExist(filename.c_str());
    };

    bool Util::deleteExist(const char *filename)
    {
    ...
    bool Util::deleteExist(const WideString & filename)
    {
      if (FileExists(filename))
      {
        DeleteFile(filename);
                                ...
                                const std::string firstColumn)
    {
      ...
      SimpleExcelAccess excel(filename, Create,
                              StringVector::
                              fromCString(6, firstColumn.c_str(), "X", "Y",
                                          "Z", "_x", "_y"));
    ...
    int_array Util::fetchVersionInfo()
    {
      ...
      DWORD dwVerInfoSize =
        GetFileVersionInfoSize(Application->ExeName.c_str(), null);
      if (dwVerInfoSize > 0)
      {
        BYTE *bVerInfoBuf = new BYTE[dwVerInfoSize];
        if (GetFileVersionInfo
            (Application->ExeName.c_str(), 0, dwVerInfoSize, bVerInfoBuf))
        {
    ...
    string_vector_ptr StringVector::fromDoubleArray(double2D_ptr doubleArray,
                                                    int n)
    {
...
#define TOKENIZER boost::tokenizer < boost::char_separator < char > >
    ...
    bptr < TStringList >
      StringVector::getTStringListFromTextFile(const std::string & filename)
    {
  ...
  DoubleBufferedCanvas::DoubleBufferedCanvas(TCanvas * canvas, int width, int height):canvas(canvas), width(width),
    ...
    bptr < Graphics::TBitmap >
      DoubleBufferedCanvas::getTBitmap(TCanvas * canvas, int width,
                                       int height)
    {
    ...
    void DoubleBufferedCanvas::copy(TCanvas * source, TCanvas * dest,
                                    int width, int height)
    {
      BitBlt(dest->Handle, 0, 0, width, height, source->Handle, 0, 0,
             SRCCOPY);
    }
