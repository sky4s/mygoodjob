...
bool Write_LUT(int *LUT, int Lut_num, TLUT Addr_LUT, WRITE_PG * write_pg)
{
	...
	if (type == 1) {	//CM 10 bit [9:2],[1:0]
		...
		LUT_W[idx] = LUT[lut_num] / 4;
		LUT_W[idx + 1] =
		    LUT[lut_num] % 4 * 64 + LUT[lut_num + 1] / 16;
		...
		LUT_W[idx + 1] =
		    LUT[lut_num] % 4 * 64 + LUT[lut_num + 1] / 16;
		LUT_W[idx + 2] =
		    LUT[lut_num + 1] % 16 * 16 + LUT[lut_num + 2] / 64;
		...
		idx = (int) floor((double) i / 4) * 5;
		LUT_W[idx] = LUT[i] % 256;
		LUT_W[idx + 1] = LUT[i] / 256 + LUT[i + 1] % 64 * 4;
		LUT_W[idx + 2] = LUT[i + 1] / 64 + LUT[i + 2] % 16 * 16;
		LUT_W[idx + 3] = LUT[i + 2] / 16 + LUT[i + 3] % 4 * 64;
		LUT_W[idx + 4] = LUT[i + 3] / 4;
	    }
	    if (remain == 1) {
		idx = (int) floor((double) lut_num / 4) * 5;
		LUT_W[idx] = LUT[lut_num] % 256;
		LUT_W[idx + 1] = LUT[lut_num] / 256;
	    } else if (remain == 2) {
		idx = (int) floor((double) lut_num / 4) * 5;
		LUT_W[idx] = LUT[lut_num] % 256;
		LUT_W[idx + 1] =
		    LUT[lut_num] / 256 + LUT[lut_num + 1] % 64 * 4;
		LUT_W[idx + 2] = LUT[lut_num + 1] / 64;
	    } else if (remain == 3) {
		idx = (int) floor((double) lut_num / 4) * 5;
		LUT_W[idx] = LUT[lut_num] % 256;
		LUT_W[idx + 1] =
		    LUT[lut_num] / 256 + LUT[lut_num + 1] % 64 * 4;
		LUT_W[idx + 2] =
		    LUT[lut_num + 1] / 64 + LUT[lut_num + 2] % 16 * 16;
		LUT_W[idx + 3] = LUT[lut_num + 2] / 16;
	    }
	    write_pg->Wrt_data = LUT_W;
	    write_pg->lut_num = Byte_num;
	}
