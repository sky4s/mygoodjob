...
TBit* Vend_12401::SetChkBx()
{
        ...
        if(MainForm->addr_place == 0){
                SetAddr(&ChkBox[0],FIFO_SYNC_RST_EN);
                SetAddr(&ChkBox[1],SDRAM_INITIAL_EN);
                SetAddr(&ChkBox[2],SSCG_FIFO_RST_EN);
                SetAddr(&ChkBox[3],CLKUS_EN);
...
TBit* Vend_12401::SetCboBx()
{
        ...
        if(MainForm->addr_place == 0){
                SetAddr(&CboBox[0],FIFO_EN);
                SetAddr(&CboBox[1],DIGITAL_FILTER_EN);
                SetAddr(&CboBox[2],XDRV_CTL);
                SetAddr(&CboBox[3],YDRV_CTL);
                SetAddr(&CboBox[4],MINILVDS_SKEW);
                SetAddr(&CboBox[5],CLKUS_WINDOW);
                SetAddr(&CboBox[6],CLKUS_THR);
        }else if(MainForm->addr_place == 1){
...
TBit* Vend_12401::SetScrollBar()
{
        ...
        if(MainForm->addr_place == 0){
                SetAddr(&ScrlB[0],KME_SDRAM_REG);
                SetAddr(&ScrlB[1],KME_REG);
        }else if(MainForm->addr_place == 1){
...
TBit* Vend_12401::SetDebugChkBx()
{
        ...
        if(MainForm->addr_place == 0){
                SetAddr(&ChkBox[0],MEM_TEST);
                SetAddr(&ChkBox[1],DIRECT_DUTY_EN);
                SetAddr(&ChkBox[2],DUTY_DETECT_TEST_EN);
...
TBit2* Vend_12401::SetDebugLblE2()
{
        ...
        if(MainForm->addr_place == 0){
                ...
                SetAddr(&LEdt[1],FRAME_CUM_RATIO);
...
TBit2* Vend_12401::SetStaLblE2()
{
        ...
        if(MainForm->addr_place == 0){
                SetAddr(&LEdt[0],BL_IDX);
                SetAddr(&LEdt[1],OUT_DUTY_DETECT_VALUE);
                SetAddr(&LEdt[2],BL_LUT_RD_DATA);
                SetAddr(&LEdt[3],DR_LUT_RD_DATA);
        }else if(MainForm->addr_place == 1){
