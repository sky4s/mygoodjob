...
namespace       jcommon {
    ...
    URL::URL(std::string url_):jcommon::Object() {
	...
	if (proto.GetSize() == 1) {
	    ...
	    throw           jcommon::
		RuntimeException("URL malformed exception");
	}

	jcommon::StringTokenizer host(proto.GetToken(proto.GetSize() - 1),
				      "/", SPLIT_FLAG, false);
	jcommon::StringTokenizer port(host.GetToken(0), ":", SPLIT_FLAG,
				      false);
	...
	if (host.GetSize() == 1) {
	    ...
	    jcommon::StringTokenizer reference(_file, "#", SPLIT_FLAG,
					       false);
	...
	while (i < numChars) {
		...
		try {
			...
			throw
			    RuntimeException
			    ("URL decoder imcomplete trailing escape '%' pattern");
		    }
		    ...
		    throw
			RuntimeException
			("URL decoder illegal hex characters in escape '%' pattern");
		}
	...
	return ((needToChange == true) ? std::string(sb) : s_);
    }
	...
	if (_protocol == "file" || _protocol == "plist"
	    || _protocol == "playlist") {
	    o << _protocol << "://" << GetPath() << std::flush;
	} else {
	    o << _protocol << "://" << _host << ":" << _port << _query <<
		std::flush;
	}
