...
namespace       jcommon {
    ...
    Calendar::Calendar():jcommon::Object() {
	jcommon::Object::SetClassName("jcommon::Calendar");

	_day = 1;
	_month = 1;
	_year = 1970;
    } Calendar::    Calendar(int day, int month,
			     int year):jcommon::Object() {
	jcommon::Object::SetClassName("jcommon::Calendar");
	...
	if (Compare(date2) < 0) {
	    ...
	    c.SetTime(date2->GetDay(), date2->GetMonth(),
		      date2->GetYear());
	...
	return ((GetDay() == date->GetDay())
		...
		&& (GetYear() == date->GetYear()))? true : false;
    }

    int             Calendar::Compare(Object * o) {
	if (o->InstanceOf("jcommon::Calendar") == false) {
	    throw          
		RuntimeException("Compare with Incompatible types");
	}
