...
# endif
	...
	} void          reset_node(const void *obj,
...
# endif
    ...
    // Pointer registation function, allocated memory must be freeable
    // using supplied destroy function
    template < typename T > T * reg(T * ptr,
				    void (*destroy) (void *, void *,
						     size_t)) {
    ...
    // Array pointer registation function, allocated memory must be
    // freeable using supplied destroy function
    template < typename T > T * reg(T arr[], size_t len,
				    void (*destroy) (void *, void *,
						     size_t)) {
  ...
  template < typename T > class const_gc_ptr:public pointer_base {
	...
	template < typename U > explicit const_gc_ptr(const U * p)	// : 
									// 
	    // 
	    // 
	    // 
	...
	template < typename U > const_gc_ptr(const U * p, const dynamic_cast_t *)	// : 
											// 
	    // 
	    // 
	    // 
	...
	template < typename U > const_gc_ptr(const U * p, const static_cast_t *)	// : 
											// 
	    // 
	    // 
	    // 
	...
	template < typename U > const_gc_ptr(const const_gc_ptr < U >
					     &p):super(p),
	                ptr(implicit_cast < T * >(p.ptr)) {
	}

	template < typename U > const_gc_ptr(const const_gc_ptr < U > &p,
					     const dynamic_cast_t
					     *):super(p),
	...
	template < typename U > const_gc_ptr < T >
	    &operator =(const const_gc_ptr < U > &p) {
	    ASSERT_VALID;
	    ptr = p.ptr;
	    return *this;
	}
	...
	template <      typename U > bool operator ==(const const_gc_ptr <
						      U > &p) const {
	    ASSERT_VALID;
	    return ptr == p.ptr;
	}
  ...
  template < typename T > class gc_ptr:public const_gc_ptr < T > {
	...
	template < typename U > gc_ptr(U * p,
				       const dynamic_cast_t
				       *):super(light_cons_t()) {
	    reset(dynamic_cast < T * >(p));
	}
	...
	template < typename U > gc_ptr(const U * p,
				       const const_cast_t
				       *):super(light_cons_t()) {
	...
	template < typename U > gc_ptr(const gc_ptr < U > &p,
				       const dynamic_cast_t *):super(p,
								     static_cast
								     <
								     dynamic_cast_t
								     *
	...
	template < typename U > gc_ptr(const const_gc_ptr < U > &p,
				       const const_cast_t *):super(p) {
	}
	...
	template < typename U > gc_ptr < T > &operator =(const gc_ptr < U >
							 &p) {
	    ASSERT_SUPER_VALID;
    ...
    /*
     * template<typename T> class const_gc_ptr: public pointer_base {
     * #ifdef _SMGC_NO_FRIEND_TEMPLATES public: #else protected:
     * template<typename U> friend class const_gc_ptr; template<typename
     * U> friend class gc_ptr; template<typename U> friend class
     * const_wk_ptr; template<typename U> friend class wk_ptr; friend
     * struct std::less<const_gc_ptr<T> >; friend struct
     * std::less<gc_ptr<T> >; #endif friend void collect();
     * 
     * T* ptr;
     * 
     * public: typedef T element_type;
     * 
     * private: typedef pointer_base super;
     * 
     * protected: template <typename U> void reset_node(const U* obj) {
     * super::reset_node(obj, destructor<U>::destroy); }
     * 
     * 
     * const_gc_ptr(const light_cons_t&) {}
     * 
     * public: const_gc_ptr(): ptr(0) {}
     * 
     * const_gc_ptr(const gc_null_t&): ptr(0) {}
     * 
     ...
     * reset(p); }
     * 
     * template<typename U> const_gc_ptr(const U *p, const
     * dynamic_cast_t*)//: ptr(0) { reset(dynamic_cast<T*>(p)); }
     * 
     * template<typename U> const_gc_ptr(const U *p, const
     * static_cast_t*)//: ptr(0) { reset(static_cast<T*>(p)); }
     * 
     * const_gc_ptr(const T *p)//: ptr(0) { reset(p); }
     * 
     * template<typename U> const_gc_ptr(const const_gc_ptr<U> &p):
     * super(p), ptr(implicit_cast<T*>(p.ptr)) {}
     * 
     * template<typename U> const_gc_ptr(const const_gc_ptr<U> &p, const
     * dynamic_cast_t*): super(p), ptr(dynamic_cast<T*>(p.ptr)) {}
     * 
     * template<typename U> const_gc_ptr(const const_gc_ptr<U> &p, const
     * static_cast_t*): super(p), ptr(static_cast<T*>(p.ptr)) {}
     * 
     * #ifndef NDEBUG ~const_gc_ptr() { ptr=0; } #endif
     * 
     * const_gc_ptr<T>& operator = (const gc_null_t&) { ASSERT_VALID; ptr
     * = 0; return *this; }
     * 
     * template<typename U> const_gc_ptr<T>& operator = (const U *p) {
     * ASSERT_VALID; reset(p); return *this; }
     * 
     * template<typename U> const_gc_ptr<T>& operator = (const
     * const_gc_ptr<U> &p) { ASSERT_VALID; ptr = p.ptr; return *this; }
     * 
     * const T& operator * () const { ASSERT_VALID; return *ptr; }
     * 
     * const T* operator -> () const { ASSERT_VALID; return ptr; }
     * 
     * const T* get() const { ASSERT_VALID; return ptr; }
     * 
     * bool operator ! () const { ASSERT_VALID; return ptr==0; }
     * 
     * operator bool () const { ASSERT_VALID; return ptr!=0; }
     * 
     * bool operator ==(const gc_null_t&) const { ASSERT_VALID; return ptr 
     * == 0; }
     * 
     * bool operator !=(const gc_null_t&) const { ASSERT_VALID; return ptr 
     * != 0; }
     * 
     * template<typename U> bool operator ==(const const_gc_ptr<U> &p)
     * const { ASSERT_VALID; return ptr == p.ptr; }
     * 
     * template<typename U> bool operator !=(const const_gc_ptr<U> &p)
     * const { ASSERT_VALID; return ptr != p.ptr; }
     * 
     ...
     * 
     * template<typename U> bool operator ==(const U *p) const {
     * ASSERT_VALID; return ptr == p; }
     * 
     * template<typename U> bool operator !=(const U *p) const {
     * ASSERT_VALID; return ptr != p; }
     * 
     * #ifdef _SMGC_PTR_COMPARABLE template<typename U> bool operator
     * <=(const U *p) const { ASSERT_VALID; return ptr <= p; }
     * 
     * template<typename U> bool operator >=(const U *p) const {
     * ASSERT_VALID; return ptr >= p; }
     * 
     ...
     * 
     * template<typename U> bool operator >(const U *p) const {
     * ASSERT_VALID; return ptr > p; } #endif
     * 
     * void reset(const T* p=0) { ASSERT_VALID; reset_node(p); ptr =
     * const_cast<T*>(p); }
     * 
     * void swap(const_gc_ptr &p) { ASSERT_VALID; T* tmp = ptr; ptr =
     * p.ptr; p.ptr = tmp; super::swap(p); } };
     * 
     * 
     * 
     * 
     ...
     * 
     * 
     * template<typename T> class gc_ptr: public const_gc_ptr<T> { public:
     * typedef T element_type;
     * 
     * private: typedef const_gc_ptr<T> super;
     * 
     * public: gc_ptr() {}
     * 
     * gc_ptr(const gc_null_t&) {}
     ...
     * reset(p); }
     * 
     * template<typename U> gc_ptr(U *p, const dynamic_cast_t*):
     * super(light_cons_t()) { reset(dynamic_cast<T*>(p)); }
     * 
     * template<typename U> gc_ptr(U *p, const static_cast_t*):
     * super(light_cons_t()) { reset(static_cast<T*>(p)); }
     * 
     * template<typename U> gc_ptr(const U *p, const const_cast_t*):
     * super(light_cons_t()) { reset(const_cast<T*>(p)); }
     * 
     * gc_ptr(T *p): super(light_cons_t()) { reset(p); }
     * 
     * template<typename U> gc_ptr(const gc_ptr<U> &p): super(p) {}
     * 
     * template<typename U> gc_ptr(const gc_ptr<U> &p, const
     * dynamic_cast_t*): super(p, static_cast<dynamic_cast_t*>(0)) {}
     * 
     * template<typename U> gc_ptr(const gc_ptr<U> &p, const
     * static_cast_t*): super(p, static_cast<static_cast_t*>(0)) {}
     * 
     * template<typename U> gc_ptr(const const_gc_ptr<U> &p, const
     * const_cast_t*): super(p) {}
     * 
     * #ifndef NDEBUG ~gc_ptr() { ASSERT_SUPER_VALID; super::ptr=0; }
     * #endif
     * 
     * gc_ptr<T>& operator = (const gc_null_t&) { ASSERT_SUPER_VALID;
     * super::ptr = 0; return *this; }
     * 
     * template<typename U> gc_ptr<T>& operator = (U *p) {
     * ASSERT_SUPER_VALID; reset(p); return *this; }
     * 
     * template<typename U> gc_ptr<T>& operator = (const gc_ptr<U> &p) {
     * ASSERT_SUPER_VALID; super::ptr = const_cast<U*>(p.ptr); return
     * *this; }
     * 
     * T& operator * () const { ASSERT_SUPER_VALID; return *super::ptr; }
     * 
     * T* operator -> () const { ASSERT_SUPER_VALID; return super::ptr; }
     * 
     * T* get() const { ASSERT_SUPER_VALID; return super::ptr; }
     * 
     * void reset(T* p=0) { ASSERT_SUPER_VALID; reset_node(p); super::ptr = 
     * p; }
     * 
     * void swap(gc_ptr &p) { ASSERT_SUPER_VALID; T* tmp = super::ptr;
     * super::ptr = p.ptr; p.ptr = tmp; super::swap(p); } };
     * 
     */
    ...
    template < typename U > bool operator ==(const gc_null_t & l,
					     const const_gc_ptr < U > &p) {
	return p.operator ==(l);
    }
    ...
    template < typename T, typename U > bool operator ==(const T * l,
							 const const_gc_ptr
							 < U > &p) {
	return p.operator ==(l);
    }
     ...
     */
    template < typename T,
	typename U > gc_ptr < T > dynamic_cast_gc_ptr(U * u) {
	return gc_ptr < T > (u, static_cast < dynamic_cast_t * >(0));
    }

    template < typename T,
	typename U > const_gc_ptr < T > dynamic_cast_gc_ptr(const U * u) {
	return const_gc_ptr < T > (u, static_cast < dynamic_cast_t * >(0));
    }

    template < typename T,
	typename U > gc_ptr < T > dynamic_cast_gc_ptr(const gc_ptr < U >
						      &u) {
	return gc_ptr < T > (u, static_cast < dynamic_cast_t * >(0));
    }

    template < typename T,
	typename U > const_gc_ptr < T >
	dynamic_cast_gc_ptr(const const_gc_ptr < U > &u) {
	return const_gc_ptr < T > (u, static_cast < dynamic_cast_t * >(0));
    }

    template < typename T,
	typename U > gc_ptr < T > static_cast_gc_ptr(U * u) {
	return gc_ptr < T > (u, static_cast < static_cast_t * >(0));
    }

    template < typename T,
	typename U > const_gc_ptr < T > static_cast_gc_ptr(const U * u) {
	return const_gc_ptr < T > (u, static_cast < static_cast_t * >(0));
    }

    template < typename T,
	typename U > gc_ptr < T > static_cast_gc_ptr(const gc_ptr < U >
						     &u) {
	return gc_ptr < T > (u, static_cast < static_cast_t * >(0));
    }
    ...
    template < typename T,
	typename U > const_gc_ptr < T >
	static_cast_gc_ptr(const const_gc_ptr < U > &u) {
	return const_gc_ptr < T > (u, static_cast < static_cast_t * >(0));
    }

    template < typename T,
	typename U > gc_ptr < T > const_cast_gc_ptr(const U * u) {
	return gc_ptr < T > (u, static_cast < const_cast_t * >(0));
    }

    template < typename T,
	typename U > gc_ptr < T > const_cast_gc_ptr(const const_gc_ptr <
						    U > &u) {
	return gc_ptr < T > (u, static_cast < const_cast_t * >(0));
    }
  ...
  template < typename T > class const_gc_arr:public pointer_base {
	...
	template <      typename U > bool operator ==(const const_gc_arr <
						      U > &p) const {
	    return ptr == p.ptr;
	}
	template <      typename U > bool operator !=(const const_gc_arr <
  ...
  template < typename T > class gc_arr:public const_gc_arr < T > {
    ...
    template < typename T > const_gc_arr < T > operator +(ptrdiff_t l,
							  const
							  const_gc_arr <
							  T > &p) {
	return p + l;
    ...
    template < typename T > gc_arr < T > operator +(ptrdiff_t l,
						    const gc_arr < T >
						    &p) {
	return p + l;
    }
  ...
  template < typename T > class const_wk_ptr:public weak_base {
	...
	template < typename U > const_wk_ptr(const const_wk_ptr < U >
					     &p):ptr(implicit_cast <
						     T * >(p.ptr)) {
	}

	template < typename U > const_wk_ptr(const const_gc_ptr < U >
					     &p):ptr(implicit_cast <
						     T * >(p.ptr)) {
	}

	template < typename U > const_wk_ptr(const const_gc_ptr < U > &p,
					     const dynamic_cast_t
					     *):ptr(dynamic_cast <
						    T * >(p.ptr)) {
	}
	...
	template < typename U > const_wk_ptr < T >
	    &operator =(const const_gc_ptr < U > &p) {
  ...
  template < typename T > class wk_ptr:public const_wk_ptr < T > {
	...
	template < typename U > wk_ptr(const wk_ptr < U > &p):super(p) {
	}

	template < typename U > wk_ptr(const gc_ptr < U > &p):super(p) {
	}

	template < typename U >
	    wk_ptr(const gc_ptr < U > &p, const dynamic_cast_t *):super(p,
									static_cast
									<
									dynamic_cast_t
									*
	...
	template < typename U >
	    wk_ptr(const gc_ptr < U > &p, const static_cast_t *):super(p,
								       static_cast
								       <
								       static_cast_t
								       *
	...
	template < typename U > wk_ptr < T > &operator =(const gc_ptr < U >
							 &p) {
	    super::ptr = const_cast < U * >(p.ptr);
	    return *this;
	}
  ...
  template < typename T > class wk_arr:public const_wk_arr < T > {
	...
	template < typename U > wk_arr(const const_gc_arr < U > &p,
				       const const_cast_t *):super(p) {
	}
    ...
    template < typename P > inline ptr_rev < P > operator+(const ptr_rev <
							   P > &p,
							   ptrdiff_t d) {
	ptr_rev < P > r = p.ptr - d;
	return r;
    ...
    template < typename P > inline ptr_rev < P > operator+(ptrdiff_t d,
							   const ptr_rev <
							   P > &p) {
	ptr_rev < P > r = d - p.ptr;
	return r;
    ...
    template < typename P > inline bool operator==(P * l,
						   const ptr_rev < P >
						   &p) {
	return p.ptr == l;
    }
	...
	len(0) {
	    ...
	    data =
		(static_cast <
		 T * >(malloc(sizeof(T) * (reserved + 1)))) + 1;
	}
	...
	fastvec(const fastvec & f):reserved(f.reserved),
	                len(f.len) {
	    data =
		(static_cast <
		 T * >(malloc(sizeof(T) * (reserved + 1)))) + 1;
	...
	fastvec & operator =(const fastvec & f) {
	    ...
	    data =
		(static_cast <
		 T * >(realloc(data - 1, sizeof(T) * (reserved + 1)))) + 1;
	...
	bool            operator() (const node_t * x, const node_t * y) const {
	    // return (static_cast<const char*>(x->base) + x->size) <=
	    // static_cast<const char*>(y->base);
	    static const    less < const void *>cmp =
		less < const void *>();
		...
		return !cmp(static_cast <
			    const char *>(y->base),
			    static_cast <
			    const char *>(x->base) + x->size);
    }} node_cmp;

    typedef         pair < pointer_base *,
                    node_t * >ptr_pair;
    typedef         pair < weak_base *,
                    node_t * >weak_pair;
    ...
    struct data_t {
	data_t():threshold(default_threshold),
	    dynamic_threshold(default_threshold),
	    allocated(0), collecting(false), current_mark(false) {
	    // InitializeCriticalSection(&cs);
	    // cs=false;
	} virtual ~     data_t() {
	    // If there is something to collect, then we do collections
	    // until
	    // nothing could be freed anymore
	    size_t          pre =
		old_nodes.size() + new_nodes.size() + born_nodes.size();
	    if (pre) {
		size_t          post = pre;
		do {
		    pre = post;
	    ...
	    // After final collection we clean up remaining undeleted
	    // nodes,
	    // but do not call destructors of objects belonging to them
	    if (pre) {
		node_vector::iterator end = old_nodes.end();
		for (node_vector::iterator i = old_nodes.begin(); i < end;
		     ++i) {
		    (*i)->destroy = 0;
		    node_t::kill(*i);
		}
		end = new_nodes.end();
		for (node_vector::iterator j = new_nodes.begin(); j < end;
		     ++j) {
		    (*j)->destroy = 0;
		    node_t::kill(*j);
		}
		end = born_nodes.end();
		for (node_vector::iterator k = born_nodes.begin(); k < end;
		     ++k) {
		    (*k)->destroy = 0;
		    node_t::kill(*k);
		}
	...
	data_lock() {
	     ...
	     *//*
	     * if(data().cs) throw "double lock"; data().cs=true;
	     */
	} virtual ~     data_lock() {
    ...
    node_t::node_t(const void *obj, /* int */ size_t node, bool intern =
		   false):         base(obj),
                    size(node),	/* mark(data().current_mark), */
	...
	if (node->object && node->destroy)
	    node->destroy(node->object, const_cast < void *>(node->base),
			  node->size);
	if (!internal)
	...
	// Construct a temporary node in order to search for the object's
	// node.
	node_t          temp(obj, 0);

	// Use lower_bound to search for the "insertion point" and
	// determine
	// if the node at this point contains the specified object.  If
	// the
	// insertion point is at the end or does not contain the object
	// then
	// we've failed to find the node and return an iterator to the
	// end.
	node_vector::iterator i =
	    lower_bound(data().new_nodes.begin(), data().new_nodes.end(),
			&temp, node_cmp);
	if (i == data().new_nodes.end() || !(*i)->contains(obj)) {
	    i = lower_bound(data().old_nodes.begin(),
			    data().old_nodes.end(), &temp, node_cmp);
	...
	for (node_vector::iterator i = data().born_nodes.begin(); i != e;
	     ++i) {
	    if ((*i)->contains(obj))
		return i;
	}
	...
	if (node && node->mark != data().current_mark) {
		...
		lower_bound(data().old_pointers.begin(),
			    data().old_pointers.end(),
			    ptr_pair(static_cast <
				     pointer_base * >(const_cast <
						      void *>(node->base)),
				     0));
    ...
    void            presort_ptr_vect(ptr_vector::iterator begin,
				     ptr_vector::iterator end) {
	static const    less < pointer_base * >cmp =
	    less < pointer_base * >();
	if (end - begin < 16)
	    return;
	...
	*begin = pivot;
	...
	while (i < j) {
	    ...
	    do {
		--j;
	    } while (cmp(piv, j->first)	/* j>begin && ... -- this is not * 
					 * needed as piv==*begin */ );
	...
	*(begin - 1) = p;	// this is perfectly legal with fastvect
    ...
    void            presort_node_vect(node_vector::iterator begin,
				      node_vector::iterator end) {
	if (end - begin < 16)
	    return;
	...
	*begin = pivot;
	...
	while (i < j) {
	    ...
	    do {
		--j;
	    } while (node_cmp(pivot, *j)	/* j>begin && ... -- this
						 * * * * * * is not needed
						 * * as * * * * piv==*begin 
						 */ );
	...
	*(begin - 1) = p;	// this is perfectly legal with fastvect
	...
	// everytime, nor we want to go for inplace_merge, as
	// it'd be slow.
	node_vector::reverse_iterator r =
	    onv.rbegin() + (nnv.size() - new_sweeped_out);
	...
	if (data().threshold != no_threshold) {
	    // Determine if we've exceeded the threshold and so should
	    // collect.
	    data().allocated += size;
	...
	// Attempt the first allocation.  The standard requires new to
	// throw
	// on failure but user code may change this behavior and VC++ by
	// default
	// only returns 0.  We'll catch exceptions and if we've already
	// collected
	...
	// If we've failed to allocate but new didn't throw an exception
	// and we've
	// not collected yet we'll collect and then re-try calling new.
	// If new
	// throws at this point we'll ignore it and let the caller handle
	// it.
	if (obj == 0 && !collected) {
	    collect();
	    obj = a.alloc(size);
	}
	// If we actually allocated memory then we need to add it to the
	// node map.
	try {
	    if (obj != 0) {
		node_t         *node;
		if (a.internal_node())
			...
			new(static_cast <
			    void *>(static_cast <
				    char *>(obj) +
				    a.node_offset(size))) node_t(obj, size,
								 true);
		else
...
# endif
	...
	pos = ps.size();
	/*
	 * if(pb.pos >= 0) ps.push_back(ptr_pair(this,
	 * ps[pb.pos].second)); else ps.push_back(ptr_pair(this,
	 * data().old_pointers[-1-pb.pos].second)); 
	 */
	ps.push_back(ptr_pair(this, 0));
    }
...
# endif
	...
	if (pos >= 0) {
		...
		if (ps[pos].first->pos != (int) (ps.size() - 1)) {
		    // cerr << "GC: memory corrupted; smartpointer lost!"
		    // << endl;
		    // abort();
		...
		if (ps[pos].first->destroyed) {
		    // cerr << "GC: memory corrupted; smartpointer
		    // destructed 2 times or overwritten!" << endl;
		    // abort();
		    throw
			mem_corrupt
			("smartpointer overwritten or double destructed");
		}
#endif
    ...
    void            pointer_base::reset_node(const void *obj,
					     void (*destroy) (void *,
							      void *,
							      size_t)) {
	data_lock       lock;
...
# endif
	...
	if (pos == (int) (ps.size() - 1)) {
		...
		throw
		    mem_corrupt
		    ("weakpointer overwritten or double destructed");
	    }
#endif
	    ps[pos].first->pos = pos;
	    ps.pop_back();
    ...
    // Register memory area in GC. Area must be freeable using supplied
    // destroy function
    void            reg(void *obj, size_t size,
			void (*destroy) (void *, void *, size_t)) {
	data_lock       lock;
  ...
  mem_corrupt::mem_corrupt(const char *txt):
	...
	msg =
	    new char[strlen("GC detected memory corruption: ") +
		     strlen(txt) + 1];
    ...
    const size_t    default_threshold =
	(INT_MAX >= 4194304 * 8) ? 4194304 : (INT_MAX / 8);


    void            collect() {
	data_lock       lock;

	// During the sweep phase we'll be deleting objects that could
	// cause
	// a recursive call to 'collect' which would cause invalid
	// results.  So
	...
	{			// Mark phase
	    // Loop through all of the pointers looking for 'root'
	    // pointers.  A 'root'
	    // pointer is a pointer that's not contained within the object 
	    // 
	    // 
	    // 
	    // 
	    // 
	    // pointed
	    // to by any other pointer.  When a 'root' pointer is found it 
	    // 
	    // 
	    // 
	    // 
	    // 
	    // is
	    // marked, and all the pointers referenced through the 'root'
	    // pointer
	    // are also marked.
	    clean_merge_ptr();
	    // sort(data().new_nodes.begin(), data().new_nodes.end(),
	    // node_cmp);
	    sort_node_vect(data().new_nodes);

	    ptr_vector::iterator end = data().old_pointers.end();
	    for (ptr_vector::iterator i = data().old_pointers.begin();
		 i != end; ++i) {
		...
		if (!node
		    || !node->
		    contains((reinterpret_cast < gc_ptr <
			      void *>&>(*ptr)).ptr)) {
		    node =
			find_node((reinterpret_cast < gc_ptr <
				   void *>&>(*ptr)).ptr);
		    // #ifndef NDEBUG
		    // if(!node)
		    // {
		    // //cerr << "Pointer points to nonsensical address"
		    // << endl;
		    // //abort();
		    // throw mem_corrupt("smartpointer points to nonsense
		    // address");
		    // }
		    // #endif
		    i->second = node;
	    ...
	    ptr_queue       que;
	    for (ptr_vector::iterator ii = data().old_pointers.begin();
		 ii != end; ++ii) {
		node_t         *node = ii->second;
		if (!node || node->mark == current_mark)
		    continue;	// Don't need to check pointers that are
		// marked.

		// If we can't find a node that contains the pointer it's
		// a root pointer
	    ...
	    for (weak_vector::iterator j = data().weak_pointers.begin();
		 j != weak_end; ++j) {
		...
		if (!node
		    || !node->
		    contains((reinterpret_cast < wk_ptr <
			      void *>&>(*ptr)).ptr)) {
		    node =
			find_node((reinterpret_cast < wk_ptr <
				   void *>&>(*ptr)).ptr);
		    j->second = node;
		}
		if (!node || node->mark != current_mark)
	...
	{			// Sweep phase
	    // Step through all of the nodes and delete any that are not
	    // marked.
	    node_vector::iterator i;
		...
		if (node->mark != current_mark) {
		    if (node->destroy == 0) {
			// We must be constructing this object, so we'll
			// just mark it.
			// This prevents premature collection of objects
			// that call
			// gc_collect during the construction phase before 
			// 
			// 
			// 
			// 
		...
		if (data().dynamic_threshold < freed / 2
		    && data().dynamic_threshold > data().threshold / 2) {
		    data().dynamic_threshold /= 2;
		} else if (data().dynamic_threshold < freed / 2
			   && data().dynamic_threshold >
			   data().threshold / 4) {
		    data().dynamic_threshold = data().threshold / 4;
		} else if (data().dynamic_threshold > remain
			   && data().dynamic_threshold >
			   data().threshold) {
...
namespace       std {
	...
	bool            operator() (const pointer_base & l,
				    const pointer_base & p) {
		...
		return
		cmp((reinterpret_cast < const const_gc_ptr <
		     void *>&>(l)).ptr,
		    (reinterpret_cast < const const_gc_ptr <
		     void *>&>(p)).ptr);
    }};
#else
	...
	bool            operator() (const jcommon::const_gc_ptr < T > &l,
				    const jcommon::const_gc_ptr < T > &p) {
	    static          less < T > cmp;
	                    ASSERT_PTR_VALID(l);
	                    ASSERT_PTR_VALID(p);
    ...
    template < typename T > struct less <jcommon::gc_ptr < T > > {
	bool            operator() (const jcommon::gc_ptr < T > &l,
				    const jcommon::gc_ptr < T > &p) {
	    static          less < T > cmp;
	                    ASSERT_PTR_VALID(l);
	                    ASSERT_PTR_VALID(p);
	                    return cmp(l.ptr, p.ptr);
    ...
    template < typename T > struct less <jcommon::const_gc_arr < T > > {
	bool            operator() (const jcommon::const_gc_arr < T > &l,
				    const jcommon::const_gc_arr < T > &p) {
	    static          less < T > cmp;
	                    ASSERT_PTR_VALID(l);
	                    ASSERT_PTR_VALID(p);
	                    return cmp(l.ptr, p.ptr);
    ...
    template < typename T > struct less <jcommon::gc_arr < T > > {
	bool            operator() (const jcommon::gc_arr < T > &l,
				    const jcommon::gc_arr < T > &p) {
	    static          less < T > cmp;
	                    ASSERT_PTR_VALID(l);
	                    ASSERT_PTR_VALID(p);
	                    return cmp(l.ptr, p.ptr);
...
void           *operator
new(size_t s, const jcommon::gc_t &)
throw(bad_alloc);
     void operator        delete(void *obj, const jcommon::gc_t &) throw();
#  ifndef _SMGC_NO_ARRAY_NEW
     void           *operator       new[] (size_t s,
					   const jcommon::
					   gc_t &) throw(bad_alloc);
     void operator        delete[] (void *obj,
				    const jcommon::gc_t &) throw();
#endif
#endif
...
void            operator
delete(void *obj, const jcommon::gc_t &)
throw()
{
...
#  ifndef _SMGC_NO_ARRAY_NEW
void           *operator       new[] (size_t s, const jcommon::gc_t &)
throw(bad_alloc)
{
    return gnew(s, jcommon::alloc_array);
}

void operator        delete[] (void *obj, const jcommon::gc_t &)
throw()
{
