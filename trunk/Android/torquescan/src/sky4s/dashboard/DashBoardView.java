...
import sky4s.dashboard.util.Interpolation;
...
class DashBoardView extends SurfaceView implements SurfaceHolder.Callback/*
	...
	class DashBoardThread extends Thread {
		...
		public int speed = 0;
		private int lastSpeed0;
		private int lastSpeed1;
		public int rpm = 0;
		private int displayRPM;
		private int lastRPM0 = -1;
		private int lastRPM1 = -1;
		...
		private void updateDashboard() {
			...
			if (lastRPM1 == -1 && lastRPM0 != 0 && rpm != lastRPM0) {
				lastRPM1 = rpm;
				return;
			}
			if (lastRPM1 != -1) {
				if (rpm == lastRPM1) {
					// 還沒更新 要內插
					double gain = ((double) System.currentTimeMillis() - lastTime1)
							/ (lastTime1 - lastTime0);
					gain = gain > 1 ? 1 : gain;
					int rpmDiff = lastRPM1 - lastRPM0;
					displayRPM = lastRPM1 + (int) (rpmDiff * gain);
				} else {
					// 更新了
					lastRPM0 = lastRPM1;
					lastRPM1 = rpm;
		...
		private void drawIndicator(Canvas canvas, Paint fpaint,
			...
			float degree = rpm2Degree(displayRPM);
