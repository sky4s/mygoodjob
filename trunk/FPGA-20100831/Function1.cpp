...
void __fastcall TFunctionForm1::FormCreate(TObject *Sender)
{
        ...
        Addr_ODLUT = OFunc->SetODLUT();         // 20100608--Set OD table Address
        ODLUT_ip_type = OFunc->ODLUT_IP_Type(); // 20100608--Set OD table type
        Initial_OD_table();                     // 20100608--Initial OD table
        ...
        for(int i = 0; i < OFunc->ODChkBox_Nbr; i++){
                ...
                if(ODChkB[i]->Addr.Name() == OFunc->OD_EN_Name())  // 20100608 find OD enable index
                        ODEN_idx = i;
        }
        if(ODEN_idx==-1)                                // 20100608 Check OD enable index
                ShowMessage("Can't Get OD enable index.");
...
//---------------------------------------------------------------------------
/*  // remove by Michelle 20100719
// 20100608 Initial OD table (AUO ip: 18x18, Vender IP: 17x17)
void TFunctionForm1::Initial_OD_table()
{
        int size;
        ...
        sg_od_1->RowCount = size+1;
        sg_od_2->RowCount = size+1;
        sg_od_1->ColCount = size+1;
        sg_od_2->ColCount = size+1;

        int step;
        for(int i = 1; i <= 17; i++){
                step = (16*(i-1))>=256?255:16*(i-1);
                sg_od_1->Cells[i][0]=IntToStr(step);
                sg_od_1->Cells[0][i]=IntToStr(step);
                sg_od_2->Cells[i][0]=IntToStr(step);
                sg_od_2->Cells[0][i]=IntToStr(step);
        }
}
*/
        ...
        if(ODLUT_ip_type==1){ // AUO IP, 18x18 table
                ...
                for(int j = 0; j < 18; j+=2){
                        ...
                        for(int i = 0; i < 18; i+=2){
                                ...
                                sg_od_2->Cells [i+2][j+1] = IntToStr((bas_1+OD_table[od1_len+81+idx])%256);
                                sg_od_2->Cells [i+1][j+2] = IntToStr((bas_2+OD_table[od1_len+81*2+idx])%256);
                                sg_od_2->Cells [i+2][j+2] = IntToStr((bas_2+OD_table[od1_len+81*3+idx])%256);
                                idx++;
                        }
                }
        }else if(ODLUT_ip_type==2){ // Vender IP
...
void __fastcall TFunctionForm1::rg_od_table_typeClick(TObject *Sender)
{
        ...
        if(ODLUT_ip_type==1){    // AUO IP
                for(int j = 0; j <= 17; j++){
                        ...
                        for(int i = 0; i <= 17; i++){
                                sg_od_1->Cells[i+1][j+1] = IntToStr((StrToInt(sg_od_1->Cells[i+1][j+1])+Ad_r*bas)%256);
                                sg_od_2->Cells[i+1][j+1] = IntToStr((StrToInt(sg_od_2->Cells[i+1][j+1])+Ad_r*bas)%256);
                        }
                }
        }else if (ODLUT_ip_type==2){ // Vender IP
                for(int j = 0; j <= 16; j++){
