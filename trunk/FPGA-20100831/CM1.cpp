...
void __fastcall TCMForm1::FormCreate(TObject *Sender)
{
        ...
        if(CMofs_type==3 || CMofs_type==1 || CMofs_type==4){ Convert_type = 1;}       // 2's completement
        else if(CMofs_type==2){ Convert_type = 2;}  // highest bit is sign bit
...
void __fastcall TCMForm1::sb_ofs1Change(TObject *Sender)
{
        ...
        if(CMofs_type==3){ s = s/2;}   //-256.5~256.5
        else if(CMofs_type==2){  s = s/8;}  //-63.875~63.875
        else if(CMofs_type==1){  s = s/4;}  //-127.75~127.75
        //else if(CMofs_type==4){ s = s;}   //-511~511
...
void TCMForm1::CM1Load(String Fpath)
{
       ...
       // Set to CM offset Interface
        int CMofs_power;
        if(CMofs_type==3){ CMofs_power = 2;}   //-256.5~256.5
        else if(CMofs_type==2){  CMofs_power = 8;}  //-63.875~63.875
        else if(CMofs_type==1){  CMofs_power = 4;}  //-127.75~127.75
        else if(CMofs_type==4){ CMofs_power = 1;}   //-511~511
