...
bool TEngineerForm::SetWrite_DG(TLUT* Addr_LUT, int** lut, int LUT_Nbr, bool IsChkSum, bool MSB_first)
{   // IsChkSum: 1 要加checksum, 0 不用加checksum
	int chk_len;
        if(IsChkSum){    // 預留的Checksum長度
                chk_len = 2;
...
bool TEngineerForm::btn_byte_readClick()
{
        ...
        char R_string[15];
        if(dif){
                if(dev_addr_cnt==2) sprintf( R_string,"%X,%X", data_read[0],data_read[1]);
                else if(dev_addr_cnt==3) sprintf( R_string,"%X,%X,%X", data_read[0],data_read[1],data_read[2]);
                else if(dev_addr_cnt==4) sprintf( R_string,"%X,%X,%X,%X", data_read[0],data_read[1],data_read[2],data_read[3]);
        }else{
...
void __fastcall TEngineerForm::btn_connect_cardClick(TObject *Sender)
{
        ...
        if(rg_i2c_card_sel->ItemIndex == 0){
                ...
                else Connect_Msg = "USB Connect Fail!";
        //LPT connect
...
bool TEngineerForm::USB_seq_pg_write(unsigned char dev_ad, unsigned char* data_addr, int data_addr_cnt,
unsigned char* data_write, int data_len, int pck_size, int wait_t)
{
        ...
        for(int i = 0; i < PAC_SIZE; i++){ data_w_tmp[i]=0; }
        if(data_w_tmp==NULL)  return 0;       //memory allocate fail
        ...
        if(tmp_len<=pck_size){
                ...
                ok = RW.USB_seq_write_P1(dev_ad_tmp, data_addr_tmp, data_addr_cnt, data_w_tmp, tmp_len, overflag);
        }else{
                overflag = 0;
                Set_write_buf(data_write, data_w_tmp, 0, pck_size);
                //Get Write data from interface
                ok = RW.USB_seq_write_P1(dev_ad_tmp, data_addr_tmp, data_addr_cnt, data_w_tmp, pck_size, overflag);
        }
...
{// EEPROM write package size = 8, and start counting address is 8 multiplier
       ...
       if(ok == 0) return 0;
       if(data_w_tmp!= NULL) delete [] data_w_tmp;
       return ok;
}
        ...
        for(int i = 0; i < dev_addr_cnt; i++){
                if(rg_i2c_card_sel->ItemIndex == 0){    //Use USB to transfer
                        ...
                        if(rg_device_addr_sel->ItemIndex == 2){
                                ...
                                if(ok == 0) ShowMessage(Err_Msg_W);
                        }else{
                        // no seperate
                                ok = USB_seq_pg_write(dev_addr[i], data_addr, data_addr_cnt, data_write, data_len, pck_size, wait_t);
                                if(ok == 0) ShowMessage(Err_Msg_W);
                        }
