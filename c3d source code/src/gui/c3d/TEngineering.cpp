...
bool TEngineerForm::pg_read(unsigned char *data_read, bool IsShow)
{
	...
	if (rg_i2c_card_sel->ItemIndex == 0) {	//USB
		...
		while (tmp_len > PAC_SIZE) {
			...
			ok = RW.USB_read(dev_addr_tmp, data_addr_tmp, data_addr_cnt, data_read_tmp,
					 PAC_SIZE);
			rd_cnt++;
		    }
		    if (rd_cnt >= 3) {
		...
		ok = 0;
		while (ok == 0 && rd_cnt < 3) {	//if fail read exceed 3, give up rework
		    ok = ok | RW.USB_read(dev_addr_tmp, data_addr, data_addr_cnt, data_read,
					  data_len);
		    rd_cnt++;
		}
		if (rd_cnt >= 3) {
...
bool TEngineerForm::pg_write()
{
	...
	if (rg_i2c_card_sel->ItemIndex == 0) {	//Use USB to transfer
	    ...
	    ok = USB_pg_write(dev_addr[i], data_addr, data_addr_cnt, data_write, data_len, pck_size,
			      wait_t);
	    // no seperate
