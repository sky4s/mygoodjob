...
bool TEngineerForm::B_write()
{
    ...
    bool card_sel;
    //根據memory決定資料的address
    if (Get_byte_addr(data_addr, &data_addr_cnt) == 0) { //取資料的address
	ShowMessage("Please enter a Byte address.");
	return 0;
    }
    //根據tcon數量決定address的byte
    if (Get_device_addr(dev_addr, &dev_addr_cnt) == 0) { //取device的address
	ShowMessage("Please choose a device address.");
	return 0;
    }

    if (Get_write_data(&write_data) == 0) { //取要寫入的資料, 只有一byte
    ...
    for (int i = 0; i < dev_addr_cnt; i++) { //隨著device address的長度改變迴圈次數
	SetDeviceAddr(dev_addr[i], &dev_addr_tmp, data_addr,
		      data_addr_cnt);
	if (rg_i2c_card_sel->ItemIndex == 0)
	    RW.USB_write(dev_addr_tmp, data_addr, data_addr_cnt,
			 &write_data, 1);
	else {
...
unsigned char TEngineerForm::B_read()
{
    ...
    int ok = 1;
    unsigned char dev_addr_tmp; //這才是真正用來寫的device address
    for (int i = 0; i < dev_addr_cnt; i++) {
        //根據memory size 和 data_addr來產生真正的device address, data_addr_cnt沒用到
	SetDeviceAddr(dev_addr[i], &dev_addr_tmp, data_addr,
		      data_addr_cnt);
	if (rg_i2c_card_sel->ItemIndex == 0)
	    connect =
		RW.USB_read(dev_addr_tmp, data_addr, data_addr_cnt,
			    &data_read, 1);
	else {
...
bool TEngineerForm::SetRead_SP(TLUT Addr_LUT, int *SP, int length)
{
    ...
    int type = Addr_LUT.Type();
    int bit_num = Addr_LUT.BitNum();
    if (type == 1 && bit_num == 4)
	for (int i = 0; i < data_len; i++) {
	    SP[i * 2] = read_data[i] % 16;
	    SP[i * 2 + 1] = read_data[i] / 16 % 16;
    } else if (type == 2 && bit_num == 4)
	for (int i = 0; i < data_len; i++)
	    SP[i] = read_data[i] % 16;
    else if (bit_num == 6)
	for (int i = 0; i < data_len; i++)
...
bool TEngineerForm::SetRead_DG(TLUT Addr_LUT, int *DG_table, int length)
{
    ...
    int idx;
    int n_idx = 0;
    if (Addr_LUT.BitNum() == 10 && Addr_LUT.Type() == 3) {
	...
	for (int i = 0; i < idx; i += 5) {
		...
		DG_table[n_idx + 1] =
		    read_data[i + 1] % 64 * 16 + read_data[i + 2] / 16;
		if (n_idx + 2 < Addr_LUT.LutNum()) {
		    DG_table[n_idx + 2] =
			read_data[i + 2] % 16 * 64 + read_data[i + 3] / 4;
		    if (n_idx + 3 < Addr_LUT.LutNum()) {
			DG_table[n_idx + 3] =
			    read_data[i + 3] % 4 * 256 + read_data[i + 4];
		    }
		}
	    }
	}
    ...
    if (Addr_LUT.BitNum() == 10 && Addr_LUT.Type() == 4) {	//未測試
	idx = ceil((double) Addr_LUT.LutNum() * 5 / 4);
	for (int i = 0; i < idx; i += 5) {
	    n_idx = floor((double) i / 5 * 4);
	    DG_table[n_idx] = read_data[i] + read_data[i + 1] % 4 * 256;
	    if (n_idx + 1 < Addr_LUT.LutNum()) {
		DG_table[n_idx + 1] =
		    read_data[i + 1] / 4 + read_data[i + 2] % 16 * 64;
		if (n_idx + 2 < Addr_LUT.LutNum()) {
		    DG_table[n_idx + 2] =
			read_data[i + 2] / 16 + read_data[i + 3] % 64 * 16;
		    if (n_idx + 3 < Addr_LUT.LutNum()) {
			DG_table[n_idx + 3] =
			    read_data[i + 3] / 64 + read_data[i + 4] * 4;
		    }
		}
	    }
	}
    } else if (Addr_LUT.BitNum() == 12 && Addr_LUT.Type() == 1) {
    ...
    } else if (Addr_LUT.BitNum() == 12 && Addr_LUT.Type() == 2) {
	idx = ceil((double) Addr_LUT.LutNum() * 3 / 2);
	n_idx = 0;
	for (int i = 0; i < idx; i += 3, n_idx += 2) {
	    DG_table[n_idx] =
		read_data[i] * 16 + read_data[i + 1] / 16 % 16;
	    if (n_idx + 1 < Addr_LUT.LutNum())
		DG_table[n_idx + 1] = read_data[i + 1] % 16 * 256;
	    if (i + 2 < idx) {
		DG_table[n_idx + 1] += read_data[i + 2];
	    }
	}
    } else
	DG_table = NULL;
...
bool TEngineerForm::SetRead_DCR(TLUT Addr_LUT, unsigned char *read_data,
				int length)
{
    String Addr_str;
    Dec2Hex(Addr_LUT.Addr(), &Addr_str);
...
bool TEngineerForm::pg_read(unsigned char *data_read, bool IsShow)
{
	...
	if (rg_i2c_card_sel->ItemIndex == 0) {	//USB
		...
		while (tmp_len > PAC_SIZE) {
		    SetDeviceAddr(dev_addr[i], &dev_addr_tmp,
				  data_addr_tmp, data_addr_cnt);
		    rd_cnt = 0;
		    ok = 0;
		    while (ok == 0 && rd_cnt < 3) {	//if fail read exceed 3, give up rework
			ok = RW.USB_read(dev_addr_tmp, data_addr_tmp,
		    ...
		    Add_val2_data_ad(data_addr_tmp, data_addr_cnt,
				     PAC_SIZE);
		...
		ok = 0;
		while (ok == 0 && rd_cnt < 3) {	//if fail read exceed 3, give up rework
		    ok = ok | RW.USB_read(dev_addr_tmp, data_addr,
...
void __fastcall TEngineerForm::btn_connect_cardClick(TObject * Sender)
{
	...
	if (rg_device_addr_sel->ItemIndex == 3) {
	    ...
	    ShowMessage
		("USB signal will be transmitted in package, Please choose one of LPT");
	    return;
	}
	cnnct_ok =
	...
	if (cnnct_ok == 0) {
	    ...
	    MessageBox(NULL, "GetProcAddress for Inp32 Failed.", "Error",
		       MB_OK);
	    return;
	} else if (cnnct_ok == -2) {
	    MessageBox(NULL, "ECR Byte Mode setting fail.", "Error",
		       MB_OK);
	    return;
	} else
	    btn_byte_readClick(Sender);
    ...
    if (connect == true) {
	btn_connect_card->Enabled = false;	//connect按鈕
